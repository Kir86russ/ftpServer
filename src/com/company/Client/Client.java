...
//			System.out.println("Data socket established to " + dataSocket.getInetAddress() + " port " + dataSocket.getPort());
            ...
            while (!userCommand.equals("quit")) {
                ...
                if (commandStrings != null && commandStrings.length > 0 && !commandStrings[0].trim().isEmpty()) { //check for blank lines
                    ...
                    switch (userCommand) {
                        case "PORT":
                            do_port(inputLine.replaceAll(" ", ""));

                            ServerSocket dataServerSocket = new ServerSocket(25061, 10, Inet4Address.getByName("192.168.1.97"));//
                            Socket dataSocket = dataServerSocket.accept();
                            //System.out.println("SERVER CONNECT FROM "+ dataSocket.getPort() +" PORT");
                            dataIs = dataSocket.getInputStream();
                            dataOs = dataSocket.getOutputStream();
                            dataScanner = new Scanner(dataIs);
                            dataWriter = new PrintWriter(dataOs, true);
                            //dataWriter.println(connectionId.toString());
                            System.out.println(ctrlScanner.nextLine());
                            System.out.println("Data socket established to " + dataSocket.getInetAddress() + " port " + dataSocket.getPort());

                            break;

                        case "USER":
                            ctrlWriter.println(inputLine.replaceAll(" ", ""));
                            ctrlWriter.flush();
                            System.out.println(ctrlScanner.nextLine());
                            break;
                        case "PASS":
                            ctrlWriter.println(inputLine.replaceAll(" ", ""));
                            ctrlWriter.flush();
                            System.out.println(ctrlScanner.nextLine() + "_230 Logged on");
                            break;
                        case "put":
    ...
    private static void do_port(String str) throws IOException {
        ctrlWriter.println(str);
        System.out.println("на клиенте написали в сокет PORT ....." + str);
    }
